...
class HomeController extends Controller
{
    ...
    public function pagu(Request $request)
    {
        ...
        Format(COALESCE(GROUP_CONCAT(dbdata.KP_REAL),0)/COALESCE(GROUP_CONCAT(dbdata.KP_PAGU),0) * 100, 2) as persenkp,
        format(GROUP_CONCAT(dbdata.DK_PAGU),0, "id_ID") DK_PAGU , format(GROUP_CONCAT(dbdata.DK_REAL),0, "id_ID") DK_REAL, 
        Format(COALESCE(GROUP_CONCAT(dbdata.DK_REAL),0)/COALESCE(GROUP_CONCAT(dbdata.DK_PAGU),0) * 100, 2) as persendk,
        format(GROUP_CONCAT(dbdata.TP_PAGU),0, "id_ID") TP_PAGU , format(GROUP_CONCAT(dbdata.TP_REAL),0, "id_ID") TP_REAL ,
        Format(COALESCE(GROUP_CONCAT(dbdata.TP_REAL),0)/COALESCE(GROUP_CONCAT(dbdata.TP_PAGU),0) * 100, 2) as persentp
