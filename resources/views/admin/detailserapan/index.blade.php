					...
					<div class="row">
						<div class="col-12">
							...
							<div class="table dataTables_wrapper dt-bootstrap4">
								<table class="table table-bordered table-striped table-hover datatable datatable-DetailBanpem w-100">
    ...
    footerCallback: function(tfoot, data, start, end, display) {
		...
		for (var i = 4; i < 9; i++) {
			...
			$(api.column(i-1).footer()).html(
				/*parseFloat(theColumnPage / columnData.count()).toFixed(2) + ' /' + */parseFloat(theColumnTotal / columnDataTotal.count()).toFixed(2) 
			);
            if ((i==8)&&(parseFloat(theColumnTotal / columnDataTotal.count()).toFixed(2)>=80) 
            &&(parseFloat(theColumnTotal / columnDataTotal.count()).toFixed(2)<=100) )
            {
                $(api.column(8).footer()).html('BAIK')    
            }
            if ((i==8)&&(parseFloat(theColumnTotal / columnDataTotal.count()).toFixed(2)>=60) 
