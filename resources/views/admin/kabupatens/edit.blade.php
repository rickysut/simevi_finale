    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.kabupatens.update", [$kabupaten->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kd_prop') ? 'is-invalid' : '' }}" name="kd_prop_id" id="kd_prop_id" required>
                    @foreach($kd_props as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kd_prop_id') ? old('kd_prop_id') : $kabupaten->kd_prop_id ?? '') == $id ? 'selected' : '' }}>{{ $id }} - {{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kd_dt1') ? 'is-invalid' : '' }}" name="kd_dt1" id="kd_dt1" >
                    @foreach($kd_dt1 as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kd_dt1') ? old('kd_dt1') : $kabupaten->kd_dt1 ?? '') == $id ? 'selected' : '' }}>{{ $id }} - {{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kd_dt2') ? 'is-invalid' : '' }}" type="text" name="kd_dt2" id="kd_dt2" value="{{ old('kd_dt2', $kabupaten->kd_dt2) }}" >
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kd_bast') ? 'is-invalid' : '' }}" type="text" name="kd_bast" id="kd_bast" value="{{ old('kd_bast', $kabupaten->kd_bast) }}" >
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kd_kemenkeu') ? 'is-invalid' : '' }}" type="text" name="kd_kemenkeu" id="kd_kemenkeu" value="{{ old('kd_kemenkeu', $kabupaten->kd_kemenkeu) }}" >
