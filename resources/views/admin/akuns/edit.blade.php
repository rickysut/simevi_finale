    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.akuns.update", [$akun->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kd_akun') ? 'is-invalid' : '' }}" type="number" name="kd_akun" id="kd_akun" value="{{ old('kd_akun', $akun->kd_akun) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_akun') ? 'is-invalid' : '' }}" type="text" name="nama_akun" id="nama_akun" value="{{ old('nama_akun', $akun->nama_akun) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pendekatan') ? 'is-invalid' : '' }}" name="pendekatan" id="pendekatan" required>
                    ...
                    @foreach(App\Models\Akun::PENDEKATAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('pendekatan', $akun->pendekatan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jenis') ? 'is-invalid' : '' }}" type="text" name="jenis" id="jenis" value="{{ old('jenis', $akun->jenis) }}" required>
                @if($errors->has('jenis'))
                    <div class="invalid-feedback">
                        {{ $errors->first('jenis') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Akun::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $akun->status) === (int) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
